...
function init() {
  ...
  notifyButton.click(()=> {
    ...
    if(playbooks.length && notification === 'playbooks') {
      rulePlaybook.subscribePlaybooksToTask(notificationResource, 'playbook-notification', playbooks, {}, states, {type:'notification'} )
      .then(task=>{
        showMessage('Started a notification playbook task ' + task.ID)
        listTasks()
      })
    } else {
      ruleBuilder.startNotificationTask(notificationResource, 'notification task', 'mandrillMail', states, {type:'notification'} )
      .then(task=>{
